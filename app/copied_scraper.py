...
def setup_logger(level: str) -> logging.Logger:
    logger = logging.getLogger("phi4mini.scraper")
    if logger.handlers:
        return logger
    lvl = getattr(logging, (level or "INFO").upper(), logging.INFO)
    logger.setLevel(lvl)
    h = logging.StreamHandler(sys.stdout)
    fmt = logging.Formatter("[%(asctime)s] [scraper] %(levelname)s: %(message)s", datefmt="%Y-%m-%dT%H:%M:%SZ")
    h.setFormatter(fmt)
    logger.addHandler(h)
...
def main() -> int:
    ...
    ap.add_argument(
        ...
        type=int,
        default=20,
        help="Max NEW items to emit per feed per run (scanner continues past already-seen items)",
    )
    ap.add_argument(
        "--scan-limit",
        type=int,
        default=500,
        help="Max total entries to scan per feed per run (safety cap)",
    )
    ...
    ap.add_argument("--debug", action="store_true", help="(deprecated) same as --log-level DEBUG")
    args = ap.parse_args()
    ...
    if not feeds:
        log.error("No feeds provided. Use --feed URL or --feeds-file feeds.txt")
        return 2

    # Sanity: at least one extractor available
    if not (HAVE_TRAFILATURA or (HAVE_READABILITY and HAVE_BS4) or HAVE_BS4):
        log.warning(
            "No HTML extractor installed. Install one of: 'pip install trafilatura' or 'pip install readability-lxml beautifulsoup4 lxml'"
        )
    ...
    for feed_url in feeds:
        log.info("feed_fetch start url=%s", feed_url)
        try:
            parsed = fetch_and_parse_feed(feed_url, session=session, timeout=args.timeout)
        except Exception as e:
            log.error("feed_fetch failed url=%s err=%s:%s", feed_url, type(e).__name__, e)
            continue
        ...
        log.info(
            "feed_parsed url=%s title=%r entries=%d scan_limit=%d max_new=%d",
            feed_url,
            feed_title,
            len(entries),
            args.scan_limit,
        ...
        if getattr(parsed, "bozo", 0):
            log.warning("feed_bozo url=%s bozo_exception=%r", feed_url, getattr(parsed, "bozo_exception", None))
        ...
        for entry in entries:
            ...
            if (not args.no_article_fetch) and link:
                log.info("item_fetch start title=%r url=%s", title[:120], link)
                ...
                if article_err:
                    log.warning("item_fetch error url=%s err=%s", link, article_err)
                else:
                    log.info("item_fetch ok url=%s text_chars=%d", link, len(article_text or ""))
            ...
            new_emitted += 1
            if args.max_items > 0 and new_emitted >= args.max_items:
                break

        log.info(
            "feed_done url=%s scanned=%d new_emitted=%d",
            feed_url,
            scanned,
            new_emitted,
        )
